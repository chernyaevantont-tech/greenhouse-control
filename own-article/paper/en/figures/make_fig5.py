...
def draw_panel(ax, d: dict, letter: str, rng) -> dict:
    ...
    # value labels.  Anchored on the whisker, but the run cloud reaches past it,
    # so each label carries a background rather than sitting among the dots.
    label_bbox = dict(boxstyle="round,pad=0.14", fc="white", ec="none", alpha=0.9)
    ax.annotate(f"{s_start['mean']:+.2f}", (xs[0], s_start["mean"] + s_start["sd"]),
                xytext=(0, 4), textcoords="offset points", ha="center", va="bottom",
                fontsize=7.5, fontweight="bold", color=C_BAR, zorder=9,
                bbox=label_bbox)
    ...
    ax.annotate(f"{d['step']:+.2f}", (xs[1], (top + bottom) / 2.0),
                ...
                color="#333333", zorder=9, bbox=label_bbox)
    ...
    for x, st in ((xs[0], s_start), (xs[2], s_end)):
        # the zero line ran through the first of the two lines
        ax.annotate(f"med {st['median']:+.2f}\n{st['wins']}/{st['n']} {kind}",
                    (x, 0.0), xytext=(0, -6), textcoords="offset points",
                    ha="center", va="top", fontsize=7, color="#444444",
                    zorder=9, bbox=dict(boxstyle="square,pad=0.15", fc="white",
                                        ec="none", alpha=0.9))
...
def main() -> int:
    ...
    axes[0].legend(handles=handles, loc="upper right", ncol=2, fontsize=7,
