...
def make_rule_based_controller(**overrides):
    """The agronomic heuristic. Called with no arguments it is the same setpoints as before.

    `**overrides` was added for N-2 (defect register, G-4): the paper calls the reference
    "tuned", but no tuning artifact exists -- the values below are hard-coded, while the
    learning controllers were given an explicit 16-trial budget. Either the search is run or
    the word "tuned" comes out. An empty call stays bit-for-bit equivalent to the previous
    behaviour, so no already-computed number moves.
    """
...
def collect_rule_based_dataset(
    ...
    # rb_params: empty -> the previous behaviour, bit for bit (N-2, see make_rule_based_controller)
    ...
    def mpc_step(state_raw: np.ndarray, step_idx: int) -> np.ndarray:
        ...
        def objective(u_flat: np.ndarray):
            ...
            for k in range(horizon):
                ...
                # N-3: the same inverted weights as in build_mpc_controller. Fixed
                # here too, or the neural MPC would be the one controller left with a
                # knowingly worse objective -- exactly the asymmetry this work charges
                # the literature with.
